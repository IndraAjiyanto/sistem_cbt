<x-main>
        <div id="menu-content" class="flex flex-col w-full pb-[30px]">
            ...
            <form action="{{route('dashboard.courses.update', $course)}}" enctype="multipart/form-data" method="post" class="flex flex-col gap-[30px] w-[500px] mx-[70px] mt-10">
                @csrf
                @method('PUT')
                <div class="flex gap-5 items-center">
                    <input type="file" name="cover" id="cover" class="peer hidden" onchange="previewFile()" data-empty="true">
                    <div class="relative w-[100px] h-[100px] rounded-full overflow-hidden">
                        ...
                        <span class="absolute transform -translate-x-1/2 -translate-y-1/2 top-1/2 left-1/2 text-center font-semibold text-sm text-[#7F8190]">Cover <br>Course</span>
                    </div>
                    <button type="button" class="flex shrink-0 p-[8px_20px] h-fit items-center rounded-full bg-[#0A090B] font-semibold text-white" onclick="document.getElementById('cover').click()">
                        Add Cover
                    </button>
                </div>
                ...
                <div class="group/category flex flex-col gap-[10px]">
                    ...
                    <div class="peer flex items-center p-[12px_16px] rounded-full border border-[#EEEEEE] transition-all duration-300 focus-within:border-2 focus-within:border-[#0A090B]">
                        ...
                        <select id="category_id" class="pl-1 font-semibold focus:outline-none w-full text-[#0A090B] invalid:text-[#7F8190] invalid:font-normal appearance-none bg-[url('{{asset('images/icons/arrow-down.svg')}}')] bg-no-repeat bg-right" name="category_id" required>
                            <option value="{{$course->category_id}}" selected hidden>{{$course->category->name}}</option>
                ...
                <div class="flex items-center gap-5">
                    ...
                    <button type="submit" name="submit" class="w-full h-[52px] p-[14px_20px] bg-[#6436F1] rounded-full font-bold text-white transition-all duration-300 hover:shadow-[0_4px_15px_0_#6436F14D] text-center">Update Course</button>
                </div>
            </form>
        </div>
</x-main>
